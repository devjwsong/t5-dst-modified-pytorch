...
def parse_data(args, from_dir, to_dir):
    ...
    id_maps = make_id_maps(args, data, valid_list, test_list)
    ...
    print("<Total Spec: MultiWoZ 2.1 (Full-shot)>")
...
def make_id_maps(args, data, valid_list, test_list, trg_domain=None):
    id_maps = {
        args.train_prefix: {slot_desc.split('-')[0]: [] for slot_desc in slot_descs},
        args.valid_prefix: {slot_desc.split('-')[0]: [] for slot_desc in slot_descs},
        args.test_prefix: {slot_desc.split('-')[0]: [] for slot_desc in slot_descs}
    }
